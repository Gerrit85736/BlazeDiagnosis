...
import { approvalStatusEnum, quoteLineCategoryEnum, quoteStatusEnum } from './enums';
...
export const quotes = pgTable(
  ...
  (table) => [index('quotes_tenant_job_idx').on(table.tenantId, table.jobCardId)],
);
...
export const quoteLineItems = pgTable(
  'quote_line_items',
  {
    ...
    total: numeric('total', { precision: 12, scale: 2 }).notNull().default('0'),
    approvalRequirement: varchar('approval_requirement', { length: 40 }).notNull().default('recommended'),
    approvalStatus: approvalStatusEnum('approval_status').notNull().default('pending'),
    dependencyGroupId: uuid('dependency_group_id').references(() => quoteDependencyGroups.id),
    laborRuleId: uuid('labor_rule_id').references(() => laborRules.id),
    sortOrder: numeric('sort_order', { precision: 8, scale: 2 }).notNull().default('0'),
    createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
  },
  (table) => [index('quote_line_items_quote_idx').on(table.tenantId, table.quoteId)],
);
...
export const quoteApprovalEvents = pgTable('quote_approval_events', {
  ...
  tenantId: uuid('tenant_id').references(() => tenants.id, { onDelete: 'cascade' }).notNull(),
  quoteId: uuid('quote_id').references(() => quotes.id, { onDelete: 'cascade' }).notNull(),
  quoteLineItemId: uuid('quote_line_item_id').references(() => quoteLineItems.id, { onDelete: 'cascade' }).notNull(),
  customerId: uuid('customer_id').references(() => customers.id).notNull(),
