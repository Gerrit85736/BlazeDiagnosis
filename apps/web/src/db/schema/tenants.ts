...
export const tenants = pgTable(
  'tenants',
  {
    ...
    timezone: varchar('timezone', { length: 80 }).notNull().default('Africa/Johannesburg'),
    currency: varchar('currency', { length: 3 }).notNull().default('ZAR'),
    createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
  },
  (table) => [uniqueIndex('tenants_slug_idx').on(table.slug), index('tenants_status_idx').on(table.status)],
);
...
export const tenantDomains = pgTable(
  ...
  (table) => [uniqueIndex('tenant_domains_domain_idx').on(table.domain), index('tenant_domains_tenant_idx').on(table.tenantId)],
);
...
export const tenantSettings = pgTable(
  ...
  (table) => [uniqueIndex('tenant_settings_unique_idx').on(table.tenantId, table.settingKey)],
);
...
export const tenantFeatureFlags = pgTable(
  ...
  (table) => [uniqueIndex('tenant_feature_flags_unique_idx').on(table.tenantId, table.flagKey)],
);
