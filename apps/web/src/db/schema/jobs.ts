...
import { jobCustomerStatusEnum, jobInternalStatusEnum, serviceRequestStatusEnum, visibilityEnum } from './enums';
...
export const serviceRequests = pgTable(
  ...
  (table) => [index('service_requests_tenant_status_idx').on(table.tenantId, table.status)],
);

export const intakeInspections = pgTable('intake_inspections', {
  id: uuid('id').defaultRandom().primaryKey(),
  tenantId: uuid('tenant_id').references(() => tenants.id, { onDelete: 'cascade' }).notNull(),
  serviceRequestId: uuid('service_request_id').references(() => serviceRequests.id, { onDelete: 'cascade' }).notNull(),
  vehicleId: uuid('vehicle_id').references(() => vehicles.id).notNull(),
...
export const jobCards = pgTable(
  'job_cards',
  {
    ...
    internalStatus: jobInternalStatusEnum('internal_status').notNull().default('request_received'),
    customerStatus: jobCustomerStatusEnum('customer_status').notNull().default('request_received'),
    ...
    assignedFloorManagerId: uuid('assigned_floor_manager_id').references(() => users.id),
    expectedCompletionAt: timestamp('expected_completion_at', { withTimezone: true }),
    completedAt: timestamp('completed_at', { withTimezone: true }),
    createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
  },
  (table) => [index('job_cards_tenant_status_idx').on(table.tenantId, table.internalStatus)],
);
