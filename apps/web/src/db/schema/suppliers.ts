...
import { deliveryStatusEnum, partsOrderStatusEnum, partsRequestStatusEnum } from './enums';
...
export const partsRequests = pgTable(
  ...
  (table) => [index('parts_requests_tenant_job_idx').on(table.tenantId, table.jobCardId)],
);

export const partsRequestItems = pgTable('parts_request_items', {
  id: uuid('id').defaultRandom().primaryKey(),
  tenantId: uuid('tenant_id').references(() => tenants.id, { onDelete: 'cascade' }).notNull(),
  partsRequestId: uuid('parts_request_id').references(() => partsRequests.id, { onDelete: 'cascade' }).notNull(),
...
export const supplierResponseItems = pgTable('supplier_response_items', {
  id: uuid('id').defaultRandom().primaryKey(),
  tenantId: uuid('tenant_id').references(() => tenants.id, { onDelete: 'cascade' }).notNull(),
  supplierResponseId: uuid('supplier_response_id').references(() => supplierResponses.id, { onDelete: 'cascade' }).notNull(),
  partsRequestItemId: uuid('parts_request_item_id').references(() => partsRequestItems.id).notNull(),
  partName: text('part_name').notNull(),
  partNumber: text('part_number'),
  brand: text('brand'),
  ...
  eta: timestamp('eta', { withTimezone: true }),
  availabilityStatus: varchar('availability_status', { length: 80 }).notNull().default('available'),
...
export const partsOrders = pgTable('parts_orders', {
  id: uuid('id').defaultRandom().primaryKey(),
  tenantId: uuid('tenant_id').references(() => tenants.id, { onDelete: 'cascade' }).notNull(),
  supplierResponseId: uuid('supplier_response_id').references(() => supplierResponses.id).notNull(),
  supplierId: uuid('supplier_id').references(() => suppliers.id).notNull(),
...
export const partsDeliveries = pgTable('parts_deliveries', {
  id: uuid('id').defaultRandom().primaryKey(),
  tenantId: uuid('tenant_id').references(() => tenants.id, { onDelete: 'cascade' }).notNull(),
  partsOrderId: uuid('parts_order_id').references(() => partsOrders.id, { onDelete: 'cascade' }).notNull(),
