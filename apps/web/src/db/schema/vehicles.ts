...
export const vehicles = pgTable(
  ...
  (table) => [index('vehicles_tenant_customer_idx').on(table.tenantId, table.primaryCustomerId)],
);
...
export const vehicleCustomers = pgTable(
  'vehicle_customers',
  {
    ...
    relationshipType: varchar('relationship_type', { length: 80 }).notNull().default('owner'),
    startedAt: timestamp('started_at', { withTimezone: true }).defaultNow().notNull(),
    endedAt: timestamp('ended_at', { withTimezone: true }),
  },
  (table) => [index('vehicle_customers_vehicle_idx').on(table.tenantId, table.vehicleId)],
);
