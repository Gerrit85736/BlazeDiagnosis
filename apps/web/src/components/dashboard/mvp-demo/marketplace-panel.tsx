...
const testParts: MarketplacePart[] = [
  {
    ...
    branches: [
      ...
      { name: 'Worldpac - Regional Distribution Center', stock: 15, eta: 'Same-day' },
    ],
  },
  {
    id: 2,
...
export function MarketplacePanel({ onSelectPart, selectedPartId }: MarketplacePanelProps) {
  const activePart = testParts.find((part) => part.id === selectedPartId) ?? testParts[0];
  ...
  return (
    <section aria-labelledby="mvp-marketplace-title" className="grid gap-4">
      <div>
        ...
        <p className="text-sm text-neutral-600">Demo supplier branch stock and ETA comparison.</p>
      </div>
      <div className="grid gap-6 rounded-xl border bg-neutral-950 p-6 text-white lg:grid-cols-[1fr_360px]">
        <div className="grid gap-4 md:grid-cols-2">
          {testParts.map((part) => {
            ...
            const totalStock = part.branches.reduce((sum, branch) => sum + branch.stock, 0);

            return (
              <button
                aria-pressed={isActive}
                className={`rounded-xl border p-5 text-left transition ${
                  isActive ? 'border-blue-300 bg-blue-500/20' : 'border-white/10 bg-white/5 hover:bg-white/10'
                }`}
                key={part.id}
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  <span className="rounded bg-blue-500 px-2.5 py-0.5 text-xs font-semibold">{part.category}</span>
        ...
        <aside className="rounded-xl border border-white/10 bg-white/5 p-5">
          <p className="text-xs font-bold uppercase tracking-wide text-blue-300">Branch sourcing</p>
          ...
          <div className="mt-4 grid gap-3">
            {activePart.branches.map((branch) => (
              <div className="rounded-lg border border-white/10 bg-white/5 p-3" key={branch.name}>
                <div className="flex justify-between gap-3 text-sm">
                  <span className="font-medium">{branch.name}</span>
                  <span className="rounded bg-neutral-900 px-2 py-0.5 text-xs">Qty: {branch.stock}</span>
                </div>
